# c-dashboard — the compiled C UI

> **Frozen.** This is where the display pipeline and touch driver were first proven against hardware, which is why it is kept rather than deleted. It gets no new features, and it is not developed toward parity with the [JS platform](../../firmware/README.md), which has since overtaken it: the platform adds networking, storage, text input, and an app model, and has nothing the dashboard has that it lacks. Hardware fixes now land in the [board sketch](../../firmware/boards/waveshare-s3-touch-147/README.md) and are not copied back here.
