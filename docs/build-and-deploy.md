...
It is also the only functional test of the binding layer, and it cannot run in CI: it executes on the board and reports over serial. CI syntax-checks every script and verifies they only call bindings the C layer registers, on a widget of a kind that has them ([`tools/check-js-api.mjs`](../tools/check-js-api.mjs)), which catches a typo, a removed binding or a method on the wrong widget, but not a behavioural regression. Now that the supervisor lives in the library rather than in each sketch, a bug there reaches every board, which makes running this by hand after binding-layer changes the actual gate.
