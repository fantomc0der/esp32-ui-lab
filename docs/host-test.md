...
Every assertion matches a whole `key=value` line rather than a substring, in both helpers (`check_printed()` for a value read back through the REPL, `expect_output()` for one a script printed). That is not fussiness: a plain substring search made "this timer fired once" pass while the timer was firing ten times, because `b=1` is a prefix of `b=10`. No current assertion has a value space wide enough for another such collision, but matching the line delimiter means the next one added cannot reintroduce it.
