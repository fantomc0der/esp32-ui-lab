...
- `sys.backlight(pct)` → LEDC PWM, clamped 0–100, hardware floor keeps the panel faintly visible. A setter only: unlike every other `sys` reading it has no getter form, and `sys.backlight()` with no argument throws a `TypeError` rather than being read as "set it to 0". There is nothing to read back, because the board's PWM is write-only and the level it applies is the one you last asked for.
