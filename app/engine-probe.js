...
// A check returns true to pass, SKIP when the feature it needs is absent, and
// anything else to fail. Skips are counted apart so the pass count means "ran".
let pass = 0, fail = 0, skip = 0;
const SKIP = "skip";
function report(name, r) {
  if (r === true) { pass++; console.log("PASS " + name); }
  else if (r === SKIP) { skip++; console.log("SKIP " + name); }
...
// Sites upstream retyped in d8e1cc6 whose effect a single-task board can observe.
...
// find_line_num decodes both the line and the column deltas through the retyped
// local, so check exact positions: the first throw is one line below a reference
// Error, and two throws on one line sit as far apart as their source text does.
check("error positions are exact (find_line_num)", () => {
  const errs = [];
  function boom() {
    errs.push(new Error("reference"));
    try { null.a; } catch (e) { errs.push(e); } try { null.b; } catch (e) { errs.push(e); }
  }
  boom();
  const pos = errs.map(e => { const m = /at boom \([^)]*:(\d+):(\d+)\)/.exec(e.stack); return m ? [+m[1], +m[2]] : null; });
  if (pos.some(p => !p)) return errs.map(e => e.stack);
  const src = boom.toString().split("\n")[2];
  const gap = src.indexOf("null.b") - src.indexOf("null.a");
  return pos[1][0] === pos[0][0] + 1 && pos[2][0] === pos[1][0] && pos[2][1] - pos[1][1] === gap ? true : pos;
});
// js_atomics_notify's retyped count is not observable here: with one task the
// waiter list is always empty, so notify returns 0 whatever count holds. This
// only proves the call works.
check("Atomics.notify is callable (retyped count not observable single-threaded)", () => {
  if (typeof Atomics === "undefined" || typeof SharedArrayBuffer === "undefined") return SKIP;
...
check("regex v flag set ops", () => {
  let r;
  try { r = new RegExp("[\\p{L}--[a-z]]", "v"); } catch (e) { return SKIP; }
  return r.test("A") && !r.test("a");
});

// Unicode tables.
check("case mapping", () => "ß".toUpperCase() === "SS" && "İ".toLowerCase().length === 2 && "ǅ".toLowerCase() === "ǆ");
check("normalize NFC/NFD", () => "é".normalize("NFD").length === 2 && "é".normalize("NFC") === "é");
// Trap 5 in docs/engine-notes.md. Fails in O(1) if the iterator stops advancing,
// where the spread below would have to exhaust the heap to show the same bug.
check("string iterator advances (Trap 5)", () => { const it = "ab"[Symbol.iterator](); it.next(); return it.next().value === "b"; });
...
check("generators", () => { function* g() { yield 1; yield 2; yield 3; } return [...g()].join() === "1,2,3"; });
check("iterator helpers", () => {
  if (typeof Iterator === "undefined") return SKIP;
  function* g() { yield 1; yield 2; yield 3; }
  return g().map(x => x * 2).toArray().join() === "2,4,6";
});
...
check("Object.groupBy", () => typeof Object.groupBy !== "function" ? SKIP : Object.groupBy([1, 2, 3], x => x % 2 ? "o" : "e").o.length === 2);
...
check("Symbol", () => typeof Symbol.iterator === "symbol" && Symbol("x").description === "x");
check("WeakRef", () => typeof WeakRef === "undefined" ? SKIP : new WeakRef({}) instanceof WeakRef);
...
function acheck(name, p) {
  asyncChecks.push(p.then(r => report(name, r), e => { fail++; console.log("FAIL " + name + " rejected " + e); }));
}
...
acheck("Array.fromAsync", typeof Array.fromAsync === "function" ? Array.fromAsync([Promise.resolve(1), 2]).then(v => v.join() === "1,2") : Promise.resolve(SKIP));
...
Promise.all(asyncChecks).then(() => {
  ...
  console.log(`ENGINEPROBE ${pass} passed, ${fail} failed, ${skip} skipped`);
});
