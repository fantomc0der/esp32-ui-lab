...
- `quickjs-ng/` — vendored JS engine (v0.15.1), Arduino library, unmodified except for one Xtensa patch. `patches/` is the source of truth for that delta and `tools/vendor-quickjs.ps1` replays it onto a new upstream; `src/` holds the already-patched code, so never hand-edit `src/` without updating the patch (why it exists: `docs/engine-notes.md`).
