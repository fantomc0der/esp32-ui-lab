...
|---|---|
| QuickJS build friction on Xtensa | **Mild.** Five one-line type fixes where a local's type and its callee's pointer parameter disagree, because this toolchain types `int32_t` as `long`. The JerryScript escape hatch was never needed. They were carried as a local patch until upstream took the same fixes in v0.17.0; the vendored engine is now unmodified. |
