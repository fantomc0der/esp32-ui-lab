...
```
        app.js                      data on SD or FATFS, never compiled
          ...
          ▼
    QuickJS-ng VM                   firmware/quickjs-ng/, heap in PSRAM
    ...
    binding layer                   firmware/lvgl-js-bindings/
       ...
       LVGL 9.5                     widget tree, layout, rendering
          ...
          ▼
  Arduino_GFX → JD9853 panel        firmware/boards/<name>/
```
...
Per-widget files were considered and rejected. `lv_binding_js` needs them because a React reconciler wants per-component prop diffing; here all seventeen widgets share one `apply_props`, so a widget is an enum value, a `switch` case, and a table row. Seventeen files of ten lines would be more structure describing less code.

That choice is what let the component runtime land in the app layer rather than in C: a reconciler needs create, set, delete and reorder, and a single shared `apply_props` already gave it the second of those for every widget at once.
...
Arguments are shaped by whether an input device is mid-dispatch. When `lv_indev_active()` returns a device, the handler receives `(widget, x, y)` in screen coordinates, which is what the Touch tab uses to put a dot under the finger, combined with `.bounds()` to convert into a container's content area. Otherwise it receives only `(widget)` — a `.value(n)` call raising `LV_EVENT_VALUE_CHANGED` from a timer, say. The test is the device and not the event, so the same `.value(n)` made *inside* a click handler produces a change event carrying the click's coordinates: `x, y` mean "where the finger is right now", which is the useful reading for placing something under it and the wrong one to read as "where this event happened".
...
2. **`lv_obj_clean(lv_screen_active())`, then reset the screen itself.** The clean deletes the widget tree, which fires `LV_EVENT_DELETE` on every widget carrying a binding, which releases those bindings *while the context is still alive*. Freeing the context before this point would mean calling `JS_FreeValue` against a dead context. What the clean does *not* touch is the screen object, and `lv.screen().set(...)` writes local styles and flags directly onto it, so a background colour or a disabled scroll would otherwise show through the next app wherever that app does not set the same property. `lv_obj_remove_style_all()` plus `lv_theme_apply()` and the three flags a script can flip put it back to what a freshly created screen has.
