...
This is the board-specific half of the firmware: pinout, hardware bring-up, the three host hooks the bindings call, and the policy choices (where scripts come from, what triggers a reload, what the serial port does). The reusable half lives in the two libraries in [`firmware/`](../../README.md). What a sketch for a different board has to supply is in [`docs/portability.md`](../../../docs/portability.md).
