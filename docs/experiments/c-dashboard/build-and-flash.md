...
`flash.ps1` in the `experiments/c-dashboard/` folder encodes all of this: `.\flash.ps1` / `-BuildOnly` / `-Port COMx` / `-Monitor`.
...
Anything else, see the troubleshooting table in the [`c-dashboard` README](../../../experiments/c-dashboard/README.md). Reopen the serial monitor after each reset — native USB re-enumerates and the old handle dies.
