// vitals-imperative.js — FROZEN BASELINE, not a script the board runs.
//
// This is app/apps/vitals.js as it was written directly against the `lv`
// bindings, kept as the "before" side of tools/test-parity.mjs. The live app is
// now built from app/src/vitals.jsx, and the parity test asserts the two build
// the same widget tree with the same props.
//
// Do not update it to match changes in the JSX version. The moment it tracks
// the port it stops being evidence of anything — it is here precisely because
// it predates the port. If the app's design genuinely moves on, delete this
// fixture and the parity test together rather than editing them into agreement.
//
// It is read as text and evaluated against tools/lv-mock.mjs, so it is never
// syntax-checked as a shipped script and never pushed to a board.

// vitals.js — the C dashboard (experiments/c-dashboard) recreated as data instead of
