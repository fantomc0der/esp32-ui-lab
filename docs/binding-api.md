...
- `sys.backlight(pct)` → LEDC PWM, clamped 0–100, hardware floor keeps the panel faintly visible. A setter only: unlike every other `sys` reading it has no getter form, so there is no `sys.backlight()`. It insists on a number, and a missing argument, `undefined`, `NaN` or a non-numeric string throws a `TypeError` instead of being read as "set it to 0", which would dim the panel to that floor, where it looks like a crashed board rather than a mistake in the script. `null` and a numeric string still convert the way JavaScript converts them, to `0` and their value. Nothing reads the level back, because the board's PWM is write-only: it is whatever was last written, starting from the brightness the board boots at rather than from anything a script chose.
