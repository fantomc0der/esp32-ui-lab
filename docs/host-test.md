...
**Wi-Fi is compiled out (`JSVM_WITH_WIFI=0`).** `WiFi.h`, `HTTPClient.h` and `NetworkClientSecure.h` are the largest stub surface in the layer and the least meaningful without a network. `sys` and `fs` *are* compiled in and covered by `test_modules`, because their stubs are small and genuinely behave: the filesystem is a real in-memory one, and `Preferences` is a real key-value store that outlives a `jsvm_stop()`/`jsvm_start()` pair the way NVS outlives a reboot.
